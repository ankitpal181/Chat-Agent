...
def clear_chat_history() -> None:
    chatbot.checkpointer.delete_thread("1")
    st.session_state["chat_logs"] = []

def load_messages(instant: bool = False, type: str = None, input: str = None) -> None:
    if instant:
        if type == "user": st.session_state["chat_logs"].append({"user": input})
        else: st.session_state["chat_logs"].append({"assistant": input})
    else:
        if "chat_logs" not in st.session_state:
            st.session_state["chat_logs"] = []
            messages = chatbot.get_state({"configurable": {"thread_id": 1}}).values.get("messages", [])

            for message in messages:
                if isinstance(message, HumanMessage):
                    st.session_state["chat_logs"].append({"user": message.content})
                elif isinstance(message, AIMessage) and message.content:
                    st.session_state["chat_logs"].append({"assistant": message.content})
                elif isinstance(message, ToolMessage) and "file_path" in message.content:
                    message_content = json.loads(message.content)
                    st.session_state["chat_logs"].append({"tool": message_content})
...
def publish_messages(instant: bool = False, type: str = None, input: str = None) -> None:
    if instant:
        ...
        for index, message in enumerate(st.session_state["chat_logs"]):
            if "user" in message:
                with st.chat_message("user"):
